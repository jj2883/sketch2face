...
parser.add_argument('--result_path', type=str, default='./results10000epoch/', help='Set the path the result images will be saved.')
...
def get_data():
    ...
    if args.task_name == 'facescrub':
        ...
        test_A = []
        test_B = []
        this_path = os.path.join("./datasets/", "facesketch")
        
        cntb = 0
        cnta = 0
        for filename in os.listdir(this_path):
            if filename == "tt":
                subdir = os.path.join(this_path, filename)
                for subfilename in os.listdir(subdir):
                    if(cnta < 200):
                        test_A.append(os.path.join(subdir, subfilename))
                        cnta += 1
                    else:
                        data_A.append(os.path.join(subdir, subfilename))
            else:
                subdir = os.path.join(this_path, filename)
                for subfilename in os.listdir(subdir):
                    if(subfilename.endswith("fa_a.ppm") or subfilename.endswith("fb_a.ppm")):
                        if(cntb < 200):
                            test_B.append(os.path.join(subdir, subfilename))
                            cntb += 1
                        else:
                            data_B.append(os.path.join(subdir, subfilename))
                    else:
                        continue     
   ...
   #     discriminator_B = discriminator_B.cuda()
    ...
    for epoch in range(epoch_size):
	    ...
	    gen_loss_B_total = gen_loss_A + recon_loss_B + idt_loss_A
            ...
            if iters % 600 == 0:
                lr = args.learning_rate * (0.95 ** (iters // 5000))
            ...
            if iters % args.log_interval == 0:
                ...
                file = open("log.txt", 'a+')
                file.write(gen_A_loss)
                file.write(",")
                file.write(gen_B_loss)
                file.write("\n")
                file.write(match_A_loss)
                file.write(",")
                file.write(match_B_loss)
                file.write("\n")
                file.write(recon_A_loss)
                file.write(",")
                file.write(recon_B_loss)
                file.write("\n")
                file.write(dis_A_loss)
                file.write(",")
                file.write(dis_B_loss)
                file.write("\n")
                file.close()
                print "---------------------"
