...
parser.add_argument('--cuda', type=str, default='false', help='Set cuda usage')
...
parser.add_argument('--result_path', type=str, default='./results_paired/', help='Set the path the result images will be saved.')
...
def get_data():
    ...
    elif args.task_name == 'sketch2face':
        data_A = []
        data_B = []
        test_A = []
        test_B = []
        test_path = "./datasets/facesketch/tt/"
	train_path = "./datasets/facesketch/"
	cnt = 0

	for filename in os.listdir(test_path):
	    cur_path = train_path + filename[:-4] + "/"
	    if(not os.path.exists(cur_path)):
		continue
	    for subfilename in os.listdir(cur_path):
		if(subfilename.startswith(filename[:-4]) and (subfilename.endswith("fb_a.ppm") or subfilename.endswith("fb.ppm"))):
		    if(cnt < 200):
			test_A.append(test_path + filename)
			test_B.append(train_path + filename[:-4] + "/" + subfilename)
		    else:
			data_A.append(test_path + filename)
			data_B.append(train_path + filename[:-4] + "/" + subfilename)
		    cnt += 1
	    	    break
...
def main():
    ...
    if args.task_name.startswith('edges2'):
        ...
        test_A = read_imgs(test_style_A, args.image_size)
        test_B = read_imgs(test_style_B, args.image_size)
    ...
    for epoch in range(epoch_size):
        ...
        for i in range(n_batches):
            ...
            if args.task_name.startswith( 'edges2' ):
                ...
                A = read_imgs( A_path , args.image_size)
                B = read_imgs( B_path , args.image_size)
            else:
	    ...
	    idt_B = generator_B(A)
            ...
            # Reconstruction Loss
            recon_loss_A = recon_criterion( ABA, A )*10
            recon_loss_B = recon_criterion( BAB, B )*10


	    idt_loss_A = idt_criterion(idt_A, B)*5
	    idt_loss_B = idt_criterion(idt_B, A)*5
	    ...
	    gen_loss_A_total = gen_loss_B + recon_loss_A + idt_loss_B
	    gen_loss_B_total = gen_loss_A + recon_loss_B + idt_loss_A
            ...
            if iters % 1000 == 0:
                lr = args.learning_rate * (0.95 ** (iters // 1000))
            ...
            if iters % args.log_interval == 0:
                gen_A_loss = as_np(gen_loss_A.mean())
                gen_B_loss = as_np(gen_loss_B.mean())
                match_A_loss = as_np(fm_loss_A.mean())
                match_B_loss = as_np(fm_loss_B.mean())
                recon_A_loss = as_np(recon_loss_A.mean())
                recon_B_loss = as_np(recon_loss_B.mean())
                dis_A_loss = as_np(dis_loss_A.mean())
                dis_B_loss = as_np(dis_loss_B.mean())
                
                file = open("log.txt", 'a+')
                file.write(str(gen_A_loss))
                file.write(",")
                file.write(str(gen_B_loss))
                file.write("\n")
                file.write(str(match_A_loss))
                file.write(",")
                file.write(str(match_B_loss))
                file.write("\n")
                file.write(str(recon_A_loss))
                file.write(",")
                file.write(str(recon_B_loss))
                file.write("\n")
                file.write(str(dis_A_loss))
                file.write(",")
                file.write(str(dis_B_loss))
                file.write("\n")
                file.close()
                
                print "---------------------"
                print "GEN Loss:", gen_A_loss, gen_B_loss
                print "Feature Matching Loss:", match_A_loss, match_B_loss
                print "RECON Loss:", recon_A_loss, recon_B_loss
                print "DIS Loss:", dis_A_loss, dis_B_loss
